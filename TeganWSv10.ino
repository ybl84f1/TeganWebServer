...
This sketch provides basic web server capabilities for the Arduino
that also provides browser access and control of your analog and digital
I/O. Basic features include:
...
TeganWS for the Arduino was written with Arduino v1.6.0.
...
#define DISPLAYLOGSIZE 4500 // number of characters in log file to display
...
void http_verb_get(char URL[]) {
  ...
  htmllist[5] = NULL; // last one MUST be NULL
  ...
  if (k > -1) {
    http_packet_read_to_end(); // read the rest of the HTTP packet...
...
void oddorevenhtml() {
  ...
  i = random(100);
  sprintf(&substitute_data[0][0], "%d", i);
  if ((i % 2) == 0) {
    strncpy(&substitute_data[1][0], "even", 5);
  } else strncpy(&substitute_data[1][0], "odd", 4);
  stream_html_file("oddoreven.html");
  return;
}
*/
